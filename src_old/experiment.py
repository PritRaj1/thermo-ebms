...
def run_experiment(exp_num, train_x, val_x, log_path):
	...
	for epoch in range(num_epochs):
		# Train
		(key, params_tup, opt_state_tup), (train_loss, train_grad, train_grad_var) = (
			jax.lax.scan(
		...
		key, fid_inf, mifid_inf, kid_inf, is_inf, four_real, four_fake = (
			jit_metrics_fcn(key, params_tup)
		)
		...
		epoch_df.to_csv(
			f"{log_path}/experiment{exp_num}.csv", mode="a", header=False, index=False
		)
