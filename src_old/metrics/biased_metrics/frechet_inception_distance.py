...
def calculate_fid(real_features, fake_features, eps=1e-4):
	...
	var_real = jnp.cov(real_features, rowvar=False) + eps * jnp.eye(
		real_features.shape[-1]
	)
	var_fake = jnp.cov(fake_features, rowvar=False) + eps * jnp.eye(
	...
	return (
		diff @ diff + jnp.trace(var_real) + jnp.trace(var_fake) - 2 * jnp.sum(covmean)
	)
